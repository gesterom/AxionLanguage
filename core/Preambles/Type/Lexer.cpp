...
std::pair<std::optional<Token>, LexerMode> Preamble::Type::Lexer::lexHead(CodeLocation& loc, uint8_t ch) {
	uint8_t last_ch = '\0';
...
std::pair<std::optional<Token>, LexerMode> Preamble::Type::Lexer::lexBody(CodeLocation& loc, uint8_t ch) {
	return { std::nullopt,LexerMode::idle };
}
